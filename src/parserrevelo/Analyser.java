...
public class Analyser {
    public static void analyser (List<String> linhas, Map<String, Jogador> players) {
    ...
    private static void analyserRegistro (Matcher RegistroPlayer, Map<String, Jogador> players) {
            ...
            if (players.containsKey(playerID)) {
                    Jogador antigo = players.get(playerID);

                    if (!antigo.getNome().equals(playerName)) {
                            players.remove(playerID);
                            antigo.setID("1000" + antigo.getID());
                            players.put(antigo.getID(), antigo);
                            players.put(playerID, new Jogador(playerID, playerName, new PlayerKD()));
                    }
            }
            else {
                    players.put(playerID, new Jogador(playerID, playerName, new PlayerKD()));
    ...
    private static void analyserKill (Matcher Kill, Map<String, Jogador> players) {
        ...
        if (!idKiller.equals(idKilled)) {
                Jogador killer = players.get(idKiller);
                if (!(killer == null)) {
        ...
        Jogador killed = players.get(idKilled);
        if (!(killed == null)) {
        ...
        if (idKiller.equals("1022")) {
                Jogador KilledWorld =  players.get(idKilled);
                if (!(KilledWorld == null)) {
